...
namespace System
{
          ...
          // IBinaryFloatParseAndFormatInfo<Half>
    {
        ...
        public static explicit operator Half(double value)
        {
            ...
            ulong doubleInt = BitConverterCompat.DoubleToUInt64Bits(value);
            bool sign = (doubleInt & DoubleHelper.SignMask) >> DoubleHelper.SignShift != 0;
            int exp = (int)((doubleInt & DoubleHelper.BiasedExponentMask) >> DoubleHelper.BiasedExponentShift);
            ulong sig = doubleInt & DoubleHelper.TrailingSignificandMask;
        ...
        public static explicit operator Half(float value)
        {
            ...
            // Exponent mask
            const uint SingleBiasedExponentMask = SingleHelper.BiasedExponentMask;
            ...
            uint bitValue = BitConverterCompat.SingleToUInt32Bits(value);
            // Extract sign bit
            uint sign = (bitValue & SingleHelper.SignMask) >> 16;
            // Detecting NaN (~0u if a is not NaN)
            // uint realMask = (uint)(Unsafe.BitCast<bool, sbyte>(float.IsNaN(value)) - 1);
            uint realMask = (uint)(float.IsNaN(value) ? 0 : -1);
            // Clear sign bit
            ...
            value = MathF.Min(MaxHalfValueBelowInfinity, value);
            // Rectify lower exponent
            uint exponentOffset0 = BitConverterCompat.SingleToUInt32Bits(MathF.Max(value, BitConverterCompat.UInt32BitsToSingle(MinExp)));
            // Extract exponent
            ...
            value += BitConverterCompat.UInt32BitsToSingle(exponentOffset0);
            bitValue = BitConverterCompat.SingleToUInt32Bits(value);
            ...
            // The final result
            return BitConverterCompat.UInt16BitsToHalf((ushort)bitValue);
        }
        ...
        public static explicit operator double(Half value)
        {
            ...
            if (exp == 0)
            {
                if (sig == 0)
                {
                    return BitConverterCompat.UInt64BitsToDouble(sign ? DoubleHelper.SignMask : 0); // Positive / Negative zero
                }
        ...
        public static explicit operator float(Half value)
        {
            ...
            const uint SingleSignMask = SingleHelper.SignMask;
            // Mask for exponent bits in Half
            ...
            short valueInInt16Bits = BitConverterCompat.HalfToInt16Bits(value);
            ...
            uint subnormalMask = (uint)((offsetExponent == 0u) ? -1 : 0);
            ...
            int infinityOrNaNMask = (offsetExponent == HalfExponentMask) ? -1 : 0;
            ...
            uint absoluteValue = BitConverterCompat.SingleToUInt32Bits(BitConverterCompat.UInt32BitsToSingle(bitValueInProcess) - BitConverterCompat.UInt32BitsToSingle(maskedExponentLowerBound));
            // Merge sign bit with rest
            return BitConverterCompat.UInt32BitsToSingle(absoluteValue | sign);
        }
        ...
        private static Half CreateHalfNaN(bool sign, ulong significand)
        {
            ...
            return BitConverterCompat.UInt16BitsToHalf((ushort)(signInt | NaNBits | sigInt));
        }
        ...
        {
            const uint NaNBits = SingleHelper.BiasedExponentMask | 0x400000; // Most significant significand bit

            uint signInt = (sign ? 1U : 0U) << SingleHelper.SignShift;
            uint sigInt = (uint)(significand >> 41);

            return BitConverterCompat.UInt32BitsToSingle(signInt | NaNBits | sigInt);
        }

        private static double CreateDoubleNaN(bool sign, ulong significand)
        {
            const ulong NaNBits = DoubleHelper.BiasedExponentMask | 0x80000_00000000; // Most significant significand bit

            ulong signInt = (sign ? 1UL : 0UL) << DoubleHelper.SignShift;
            ulong sigInt = significand >> 12;

            return BitConverterCompat.UInt64BitsToDouble(signInt | NaNBits | sigInt);
        }

        private static float CreateSingle(bool sign, byte exp, uint sig) => BitConverterCompat.UInt32BitsToSingle(((sign ? 1U : 0U) << SingleHelper.SignShift) + ((uint)exp << SingleHelper.BiasedExponentShift) + sig);

        private static double CreateDouble(bool sign, ushort exp, ulong sig) => BitConverterCompat.UInt64BitsToDouble(((sign ? 1UL : 0UL) << DoubleHelper.SignShift) + ((ulong)exp << DoubleHelper.BiasedExponentShift) + sig);
        ...
        public static Half Log2(Half value) => (Half)MathF.Log((float)value, 2.0f);
        ...
        public static Half Atan2Pi(Half y, Half x) => throw new NotImplementedException();
        ...
        public static Half FusedMultiplyAdd(Half left, Half right, Half addend) => throw new NotImplementedException();
        ...
        public static Half Lerp(Half value1, Half value2, Half amount) => throw new NotImplementedException();

        /// <inheritdoc cref="IFloatingPointIeee754{TSelf}.ReciprocalEstimate(TSelf)" />
        public static Half ReciprocalEstimate(Half x) => throw new NotImplementedException();

        /// <inheritdoc cref="IFloatingPointIeee754{TSelf}.ReciprocalSqrtEstimate(TSelf)" />
        public static Half ReciprocalSqrtEstimate(Half x) => throw new NotImplementedException();

        /// <inheritdoc cref="IFloatingPointIeee754{TSelf}.ScaleB(TSelf, int)" />
        public static Half ScaleB(Half x, int n) => throw new NotImplementedException();
        ...
        public static Half LogP1(Half x) => throw new NotImplementedException();

        /// <inheritdoc cref="ILogarithmicFunctions{TSelf}.Log2P1(TSelf)" />
        public static Half Log2P1(Half x) => throw new NotImplementedException();

        /// <inheritdoc cref="ILogarithmicFunctions{TSelf}.Log10P1(TSelf)" />
        public static Half Log10P1(Half x) => throw new NotImplementedException();
